...
{
    public class EncryptedIntegerBinder : IModelBinder
    {
        private IEncryptionService _EncryptionService;

        public EncryptedIntegerBinder()
            : this(new FrontEndEnrypter())
        {
        }

        public EncryptedIntegerBinder(IEncryptionService encryptionService)
        {
        ...
        public virtual object BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
        {
            ...
            if ((val != null) && !string.IsNullOrEmpty(val.AttemptedValue))
            {
                ...
                else
                {
                    ...
                    value = Convert.ToInt32(decryptedValue);
                }
            }
            else
                value = null;
