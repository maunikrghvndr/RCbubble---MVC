...
<%
    var grid = Html.DevExpress().GridView(settings =>
    {
            ...
            {   
                ...
                ViewContext.Writer.Write(
                  @Html.DevExpress().Image(
                           Imgsettings =>
                           {
                               Imgsettings.Name = "SentAction" + c.VisibleIndex;
        ...
        settings.Columns.Add(column =>
        {
            ...
            column.SetDataItemTemplateContent(c =>
            {
                ...
                if (!string.IsNullOrEmpty(createdUTCDate))
                {
                    ...
                    {
                        lblSettings.Name = c.VisibleIndex + "lblSentOn_" + c.VisibleIndex;
        ...
        settings.Columns.Add(column =>
        {
            ...
            column.SetDataItemTemplateContent(c =>
            {
               //ViewContext.Writer.Write(Html.ActionLink(Convert.ToString(DataBinder.Eval(c.DataItem, "ENoteID")), "AccountDetailsMaster", "AccountDetails", new { eNoteID = Convert.ToString(DataBinder.Eval(c.DataItem, "ENoteID")), TCNIdentificationID = Html.Encrypt(Convert.ToString(DataBinder.Eval(c.DataItem, "TCNIdentificationID"))), AccountNo = Convert.ToString(DataBinder.Eval(c.DataItem, "AccountNo")), SenderProviderID = Html.Encrypt(Convert.ToString(DataBinder.Eval(c.DataItem, "SenderProviderId"))), TCNNo = Convert.ToString(DataBinder.Eval(c.DataItem, "TCNNo")), AccountSubmissionDetailsID = Html.Encrypt(Convert.ToString(DataBinder.Eval(c.DataItem, "AccountSubmissionDetailsID"))), DeadLineDate = Convert.ToString(DataBinder.Eval(c.DataItem, "DocumentDeadLine")), sourceFlag = 4  }, new { onclick = " generalizedShowLoader();" }));
        ...
        //settings.ClientSideEvents.Init = "function(s,e){alert('hi');}";
        //settings.ClientSideEvents.Init = "function (s, e){ oneEndCallback(s, e," + Model.DeadLineRecordCount + ", " + Model.TwoDaysToDeadLineRecordCount + ", " + Model.ThreeDaysToDeadLineRecordCount + ", " + Model.MoreThanThreeDaysToDeadLineRecordCount + " );}";
        ...
        settings.ClientSideEvents.BeginCallback = "onBeingCallback_SentGrid";
    });
